...
/**
 * One consultant with agency org, a fully entitled (Enterprise-tier) managed client,
 * and comprehensive data across every module — locations, Scope 1/2/3 emissions,
 * disclosures (IFRS S1/S2, GRI, UAE ESG), climate risk register, reduction targets,
 * ESG KPIs, HRIS import log, SASB sector — for full end-to-end product demos/testing.
 *
...
class ConsultantFullDemoSeeder extends Seeder
{
    ...
    private const TOP_CONSULTANT_PACK = 'consultant_50';

    private const TOP_COMPANY_PLAN = 'client_enterprise';
    ...
    public function run(): void
    {
        ...
        $subscription = $this->ensureTopTierSubscription($consultantOrg, $reportingYear);
        $engagement = $this->ensureEngagement($consultantOrg, $reportingYear);
        $managed = $engagement->managedCompany;

        $this->enrichManagedCompanyProfile($managed);
        ...
        $reductionTarget = $this->seedReductionTargets($managed, $reportingYear);
        ...
        $this->printSummary($consultantOrg, $managed, $locations, $reportingYear, $totalEntries, $reductionTarget);
    }
    ...
    private function ensureConsultant(): Consultant
    {
        ...
        return Consultant::create([
            ...
            'bio' => 'Demo consultant account for testing managed client workspaces, emissions entry, disclosures, and the client interface from the agency hub.',
    ...
    /**
     * Grants the top-tier consultant agency pack (consultant_50) with the managed-client
     * package pinned to client_enterprise, so the managed client gets full Scope 3,
     * all export formats, and every disclosure module unlocked.
     */
    private function ensureTopTierSubscription(Company $consultantOrg, int $reportingYear): ConsultantSubscription
    {
        $existing = ConsultantSubscription::forConsultant($consultantOrg->id)
            ->active()
            ->whereHas('plan', fn ($q) => $q->where('plan_code', self::TOP_CONSULTANT_PACK))
            ->get()
            ->first(fn (ConsultantSubscription $sub) => ($sub->metadata['managed_client_package_code'] ?? null) === self::TOP_COMPANY_PLAN);

        if ($existing) {
            return $existing;
        }

        return app(ConsultantAgencySubscriptionService::class)->grantPackSubscription(
            $consultantOrg,
            self::TOP_CONSULTANT_PACK,
            $reportingYear,
            [
                'managed_client_package_code' => self::TOP_COMPANY_PLAN,
                'provision_note' => 'Full demo seed — Enterprise-tier managed client access',
            ],
        );
    }

    private function ensureEngagement(Company $consultantOrg, int $reportingYear): ConsultantClientEngagement
    {
        ...
        if ($existing) {
            $existing->loadMissing('managedCompany');

            return $existing;
        }

        return app(ConsultantAgencyClientService::class)->create($consultantOrg, [
            ...
            'description' => 'Demo managed client — fully populated for end-to-end testing and client demos.',
        ]);
    }

    private function enrichManagedCompanyProfile(Company $managed): void
    {
        $managed->update([
            ...
            'city' => 'Dubai',
            'postal_code' => '00000',
            'website' => 'https://alnoortrading.example.ae',
            'business_subcategory' => 'Import & Distribution',
            'employee_count' => 180,
    ...
    private function seedAdminAssignment(
        ...
        AdminPackageAssignment::firstOrCreate(
            ...
            [
                ...
                'duration_months' => 12,
                'note' => 'Full demo seed — consultant_50 pack with client_enterprise managed-client access.',
                'status' => 'approved',
    ...
    private function ensureConsultantAgencyPlans(): void
    {
        ...
        $top = SubscriptionPlan::where('plan_code', self::TOP_CONSULTANT_PACK)->first();

        if (!$top?->isConsultantAgencyPack()) {
            throw new RuntimeException(
                self::TOP_CONSULTANT_PACK . ' plan is missing or has wrong plan_category. Expected plan_category=consultant_agency.'
            );
        }
    ...
    private function printSummary(
        ...
        $this->command?->info('');
        $this->command?->info('✅ Full consultant demo seeded (Enterprise-tier managed client)');
        $this->command?->table(
            ['Item', 'Value'],
            [
                ...
                ['Consultant pack', self::TOP_CONSULTANT_PACK . ' (50 client slots)'],
                ['Agency org', $consultantOrg->name . ' (id ' . $consultantOrg->id . ')'],
                ['Managed client', $managed->name . ' (id ' . $managed->id . ')'],
                ['Managed client plan', self::TOP_COMPANY_PLAN . ' (full Scope 3, all exports, all disclosures)'],
                ['Reporting year (PRY)', (string) $reportingYear],
                ['Locations', collect($locations)->pluck('name')->implode(', ')],
                ['Emission entries (all sites)', (string) $totalEntries],
                ['Disclosures seeded', 'IFRS S2, IFRS S1, GRI, UAE ESG report'],
                ['Climate risk register', '3 risks, 3 opportunities'],
                ['Reduction target', $reductionTarget?->name ?? '—'],
                ['ESG KPI snapshots', 'Environment, Social, Governance (manual + auto + HRIS)'],
                ['HRIS import log', 'Seeded (Workday, 11 rows)'],
                ['SASB sector', 'TR-RO (Road Transportation)'],
            ]
        );
        $this->command?->info('Test flow: sign in → Dashboard → Open workspace → Quick Input, Disclosures, Reports, ESG Scorecard.');
        $this->command?->info('Re-run this seeder anytime to backfill any missing data — all steps are idempotent.');
    }
}
