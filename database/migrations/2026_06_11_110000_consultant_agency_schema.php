...
return new class extends Migration
{
    public function up(): void
    {
        if (Schema::hasTable('consultant_subscriptions')) {
            $this->ensureConsultantColumns();

            return;
        }

        if (Schema::hasTable('partner_subscriptions')) {
            // 2026_06_11_105000_rename_partner_schema_to_consultant runs first and handles live DB.
            return;
        }
        ...
        if (!Schema::hasTable('consultant_subscription_addons')) {
    ...
    private function ensureConsultantColumns(): void
    {
        ...
        if (Schema::hasTable('companies')
            && Schema::hasColumn('companies', 'partner_id')
            && !Schema::hasColumn('companies', 'consultant_id')) {
            Schema::table('companies', function (Blueprint $table) {
                $table->renameColumn('partner_id', 'consultant_id');
            });
        }

        if (Schema::hasTable('consultants')
            && Schema::hasColumn('consultants', 'partner_company_id')
            && !Schema::hasColumn('consultants', 'agency_company_id')) {
            Schema::table('consultants', function (Blueprint $table) {
                $table->renameColumn('partner_company_id', 'agency_company_id');
            });
        }
    }
