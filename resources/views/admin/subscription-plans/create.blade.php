...
@section('title', 'Create Subscription Plan | MENetZero')
        ...
        <form method="POST" action="{{ route('admin.subscription-plans.store') }}">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" name="plan_code" id="plan_code" value="{{ old('plan_code') }}" required
                ...
                <div>
                    ...
                    <select name="plan_category" id="plan_category" required
                            class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-purple-500 focus:border-purple-500">
                        <option value="client" {{ old('plan_category') == 'client' ? 'selected' : '' }}>Client</option>
                    </select>
                    @error('plan_category')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <input type="text" name="currency" id="currency" value="{{ old('currency', 'AED') }}" maxlength="3" required
                           class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-purple-500 focus:border-purple-500">
                    @error('currency')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
            ...
            <div class="mt-6">
                ...
                <textarea name="features" id="features" rows="4" placeholder='["feature1", "feature2"]'
                          class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-purple-500 focus:border-purple-500 font-mono text-sm">{{ old('features') ? json_encode(old('features'), JSON_PRETTY_PRINT) : '[]' }}</textarea>
            ...
            <div class="mt-6">
                ...
                <textarea name="limits" id="limits" rows="4" placeholder='{"locations": 5, "users": 3}'
                          class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-purple-500 focus:border-purple-500 font-mono text-sm">{{ old('limits') ? json_encode(old('limits'), JSON_PRETTY_PRINT) : '{}' }}</textarea>
