...
@section('title', 'Create Unit Conversion | MENetZero')
@section('page-title', 'Create Unit Conversion')

@section('content')
    <div class="bg-white shadow rounded-lg p-6">
        <form method="POST" action="{{ route('admin.emissions.unit-conversions.store') }}">
            @csrf

            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    <label for="from_unit" class="block text-sm font-medium text-gray-700 mb-1">From Unit *</label>
                    <input type="text" name="from_unit" id="from_unit" value="{{ old('from_unit') }}" required
                ...
                <div>
                    ...
                    <input type="text" name="to_unit" id="to_unit" value="{{ old('to_unit') }}" required
                           class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-purple-500 focus:border-purple-500">
                    @error('to_unit')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <input type="number" step="0.000001" name="conversion_factor" id="conversion_factor" value="{{ old('conversion_factor') }}" required
                           class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-purple-500 focus:border-purple-500">
