...
@section('title', 'Create Emission Factor | MENetZero')
@section('page-title', 'Create Emission Factor')

@section('content')
    <div class="bg-white shadow rounded-lg p-6">
        <form method="POST" action="{{ route('admin.emissions.factors.store') }}">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="number" step="0.000001" name="factor_value" id="factor_value" value="{{ old('factor_value') }}" required
                           class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-purple-500 focus:border-purple-500">
                    @error('factor_value')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <input type="text" name="region" id="region" value="{{ old('region', 'UAE') }}"
                           class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-purple-500 focus:border-purple-500">
                </div>

                <div>
                    <label for="co2_factor" class="block text-sm font-medium text-gray-700 mb-1">CO2 Factor</label>
                    <input type="number" step="0.000001" name="co2_factor" id="co2_factor" value="{{ old('co2_factor') }}"
                           class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-purple-500 focus:border-purple-500">
                </div>

                <div>
                    <label for="ch4_factor" class="block text-sm font-medium text-gray-700 mb-1">CH4 Factor</label>
                    <input type="number" step="0.000001" name="ch4_factor" id="ch4_factor" value="{{ old('ch4_factor') }}"
                           class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-purple-500 focus:border-purple-500">
                </div>

                <div>
                    <label for="n2o_factor" class="block text-sm font-medium text-gray-700 mb-1">N2O Factor</label>
                    <input type="number" step="0.000001" name="n2o_factor" id="n2o_factor" value="{{ old('n2o_factor') }}"
                           class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-purple-500 focus:border-purple-500">
                </div>

                <div>
                    <label for="total_co2e_factor" class="block text-sm font-medium text-gray-700 mb-1">Total CO2e Factor</label>
                    <input type="number" step="0.000001" name="total_co2e_factor" id="total_co2e_factor" value="{{ old('total_co2e_factor') }}"
                           class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-purple-500 focus:border-purple-500">
                </div>
