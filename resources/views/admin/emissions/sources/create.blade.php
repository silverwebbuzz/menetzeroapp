...
@section('title', 'Create Emission Source | MENetZero')
@section('page-title', 'Create Emission Source')

@section('content')
    <div class="bg-white shadow rounded-lg p-6">
        <form method="POST" action="{{ route('admin.emissions.sources.store') }}">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select name="scope" id="scope" required
                            class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-purple-500 focus:border-purple-500">
                        <option value="1" {{ old('scope') == '1' ? 'selected' : '' }}>Scope 1</option>
                        <option value="2" {{ old('scope') == '2' ? 'selected' : '' }}>Scope 2</option>
                        <option value="3" {{ old('scope') == '3' ? 'selected' : '' }}>Scope 3</option>
                    </select>
                    @error('scope')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <input type="text" name="quick_input_slug" id="quick_input_slug" value="{{ old('quick_input_slug') }}" placeholder="e.g., natural-gas"
                           class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-purple-500 focus:border-purple-500">
                ...
                <div>
                    ...
                    <input type="number" name="quick_input_order" id="quick_input_order" value="{{ old('quick_input_order', 0) }}" min="0"
                           class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-purple-500 focus:border-purple-500">
                </div>

                <div class="flex items-center">
                    <input type="checkbox" name="is_quick_input" id="is_quick_input" value="1" {{ old('is_quick_input') ? 'checked' : '' }}
