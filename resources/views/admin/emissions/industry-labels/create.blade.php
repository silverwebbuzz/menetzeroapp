...
@section('title', 'Create Industry Label | MENetZero')
@section('page-title', 'Create Industry Label')

@section('content')
    <div class="bg-white shadow rounded-lg p-6">
        <form method="POST" action="{{ route('admin.emissions.industry-labels.store') }}">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select name="industry_category_id" id="industry_category_id"
                        ...
                        @foreach($categories as $category)
                            <option value="{{ $category->id }}" {{ old('industry_category_id') == $category->id ? 'selected' : '' }}>{{ $category->name }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="user_friendly_name" id="user_friendly_name" value="{{ old('user_friendly_name') }}" required
                           class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-purple-500 focus:border-purple-500">
                ...
                <div>
                    ...
                    <select name="match_level" id="match_level"
                            class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-purple-500 focus:border-purple-500">
                        <option value="">None</option>
                        <option value="1" {{ old('match_level') == '1' ? 'selected' : '' }}>Level 1 (Sector)</option>
                        <option value="2" {{ old('match_level') == '2' ? 'selected' : '' }}>Level 2 (Industry)</option>
                        <option value="3" {{ old('match_level') == '3' ? 'selected' : '' }}>Level 3 (Subcategory)</option>
                    </select>
                </div>

                <div>
                    <label for="unit_type" class="block text-sm font-medium text-gray-700 mb-1">Unit Type</label>
                    <input type="text" name="unit_type" id="unit_type" value="{{ old('unit_type') }}" placeholder="e.g., Main Factory, Office Building"
                           class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-purple-500 focus:border-purple-500">
                </div>
                ...
                <div class="flex items-center">
                    <input type="checkbox" name="also_match_children" id="also_match_children" value="1" {{ old('also_match_children') ? 'checked' : '' }}
            ...
            <div class="mt-6">
                ...
                <textarea name="user_friendly_description" id="user_friendly_description" rows="3"
                          class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-purple-500 focus:border-purple-500">{{ old('user_friendly_description') }}</textarea>
            </div>

            <div class="mt-6">
                <label for="typical_units" class="block text-sm font-medium text-gray-700 mb-1">Typical Units</label>
                <input type="text" name="typical_units" id="typical_units" value="{{ old('typical_units') }}"
                       class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-purple-500 focus:border-purple-500">
            </div>
