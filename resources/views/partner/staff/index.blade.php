...
<div class="max-w-7xl mx-auto">
    ...
    <div class="flex justify-end mb-6">
        <a href="{{ route('partner.staff.create') }}" class="inline-flex items-center gap-2 px-6 py-3 border border-orange-500 text-orange-600 bg-white rounded-lg hover:bg-orange-50 transition">
        ...
        <div class="space-y-3">
            ...
            <div class="flex items-center justify-between bg-white rounded-lg p-4 border border-yellow-200">
                <div>
                    ...
                    <form action="{{ route('partner.staff.cancel-invitation', $invitation) }}" method="POST" class="inline">
                    @csrf
                    @method('DELETE')
                    ...
                    <tr>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="text-sm text-gray-900">
                                {{ $access->customRole->role_name ?? ($access->role->name ?? 'N/A') }}
                            </div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full 
                                {{ $access->access_level === 'full' ? 'bg-green-100 text-green-800' : 
                                   ($access->access_level === 'edit' ? 'bg-blue-100 text-blue-800' : 'bg-gray-100 text-gray-800') }}">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                            ...
                            <form action="{{ route('partner.staff.destroy', $access) }}" method="POST" class="inline">
                                @csrf
                                @method('DELETE')
