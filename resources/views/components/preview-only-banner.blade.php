...
<div {{ $attributes->merge(['class' => 'mb-4 rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3']) }}>
    ...
    <a href="{{ route('subscriptions.upgrade') }}" class="inline-flex items-center justify-center px-4 py-2 text-sm font-semibold text-white bg-amber-600 rounded-lg hover:bg-amber-700 whitespace-nowrap">
        {{ $upgradeLabel }}
    </a>
</div>
