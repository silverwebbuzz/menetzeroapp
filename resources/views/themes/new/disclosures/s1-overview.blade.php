{{--
    MENetZero 2.0 - IFRS S1 Disclosures (Phase 6 body migration).

    PLAN GATING preserved: the PDF export is wrapped in x-plan-gated-link with
    $gate->canDisclosureExportType('ifrs_s1_pdf', $fiscalYear). This is a paid
    ...
    <div class="mnz-panel">
        <div class="mnz-panel__head">
            <div>
                ...
                <p style="font-size:12px;color:var(--ink-3);margin:3px 0 0">IFRS S1 general sustainability pillars for {{ $fiscalYear }}.</p>
            </div>
    ...
    <div class="mnz-panel">
        <div class="mnz-panel__body" style="display:flex;gap:16px;align-items:center;justify-content:space-between;flex-wrap:wrap">
            <div style="min-width:min(100%,300px)">
                <h4 style="font-size:13px;font-weight:600;margin:0">IFRS S1 report</h4>
                <p style="font-size:12.5px;color:var(--ink-3);margin:4px 0 0">Includes IFRS S2 climate appendix when climate data exists.</p>
            </div>
            <div style="display:flex;gap:8px">
                <a href="{{ route('disclosures.s1.report.preview', ['fiscal_year' => $fiscalYear]) }}" class="mnz-btn">Preview</a>
                <x-plan-gated-link
