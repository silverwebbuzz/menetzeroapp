{{--
    MENetZero 2.0 - Add New Role (Phase 6 body migration).
    ...
    Controller data: $permissions (grouped by module) + TeamAccessService shared
--}}
            ...
            <form action="{{ route(($teamRoutes ?? [])['roles.store'] ?? 'roles.store') }}" method="POST" id="roleForm">
                ...
                <div class="mnz-field" style="margin-bottom:16px">
                    ...
                    <input type="text" name="role_name" id="role_name" required
                           value="{{ old('role_name') }}" placeholder="Enter a role name" class="mnz-input">
                ...
                <div class="mnz-field" style="margin-bottom:18px">
                    ...
                    <textarea name="description" id="description" rows="3"
                              placeholder="Enter role description" class="mnz-textarea">{{ old('description') }}</textarea>
                </div>
                ...
                <div class="rl-matrix">
                    ...
                    @foreach($permissions as $module => $modulePermissions)
                        ...
                        <div class="rl-matrix__row">
                            ...
                            <div class="t-c">
                                @if($viewPerm)
                                    <input type="checkbox" name="permission_ids[]" value="{{ $viewPerm->id }}" class="module-checkbox">
                                @endif
                            </div>
                            <div class="t-c">
                                @if($addPerm)
                                    <input type="checkbox" name="permission_ids[]" value="{{ $addPerm->id }}" class="module-checkbox">
                                @endif
                            </div>
                            <div class="t-c">
                                @if($editPerm)
                                    <input type="checkbox" name="permission_ids[]" value="{{ $editPerm->id }}" class="module-checkbox">
                                @endif
                            </div>
                            <div class="t-c">
                                @if($deletePerm)
                                    <input type="checkbox" name="permission_ids[]" value="{{ $deletePerm->id }}" class="module-checkbox">
                                @endif
                            </div>
                        </div>
                    @endforeach
