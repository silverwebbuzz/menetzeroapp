...
<div class="w-full">
    ...
    <div class="card mb-6">
                    ...
                    <tbody>
                        @foreach($risks as $risk)
                            <tr class="border-b border-gray-100">
                                ...
                                <td class="px-4 py-3 text-gray-600">{{ $risk->topicLabel() }}</td>
                                <td class="px-4 py-3 text-gray-600">{{ \App\Models\SustainabilityRisk::HORIZONS[$risk->time_horizon] ?? $risk->time_horizon }}</td>
                                <td class="px-4 py-3 text-right">
                                    @if($risk->likelihood)
                                        <span class="inline-block text-xs px-2 py-0.5 rounded border {{ $likelihoodBadge[$risk->likelihood] ?? 'bg-gray-50 text-gray-600 border-gray-200' }}">
                                            {{ \App\Models\SustainabilityRisk::LIKELIHOODS[$risk->likelihood] ?? $risk->likelihood }}
                                        </span>
                                    @else
        ...
        @else
            <div class="card-body"><p class="text-gray-500 text-sm">No sustainability risks for {{ $fiscalYear }} yet.</p></div>
        @endif
    </div>
            ...
            <form method="POST" action="{{ route('disclosures.s1.sustainability-risks.store', ['fiscal_year' => $fiscalYear]) }}" class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <select name="likelihood" class="w-full border border-gray-300 rounded-lg px-3 py-2">
                        ...
                        @foreach(\App\Models\SustainabilityRisk::LIKELIHOODS as $value => $label)
                ...
                <div class="md:col-span-2">
                    ...
                    <p class="text-xs text-gray-500 mt-1">IFRS S1 asks for the anticipated financial effect. A range is acceptable.</p>
                </div>
                <div class="md:col-span-2">
    ...
    @foreach($risks as $risk)
        <details id="risk-{{ $risk->id }}" class="card mb-3">
            <summary class="card-body font-medium cursor-pointer flex justify-between items-center">
                <span>{{ $risk->name }}</span>
                <span class="text-xs text-gray-500">{{ $risk->topicLabel() }}</span>
            </summary>
            <div class="card-body border-t border-gray-100">
                <form method="POST" action="{{ route('disclosures.s1.sustainability-risks.update', ['sustainabilityRisk' => $risk, 'fiscal_year' => $fiscalYear]) }}" class="grid grid-cols-1 md:grid-cols-2 gap-3">
                    @csrf @method('PUT')
                    <input type="hidden" name="fiscal_year" value="{{ $fiscalYear }}">
